...
public class BST<T extends Comparable<? super T>> {
	...
	public int countNode(BSTNode<T> root) {
	...
	// Exercise 4 - Helper method
	public int findingheight(BSTNode<T> root) {
		
		int lH = 0;
		int rH = 0;
		
		if(root.left != null) {
			lH = findingheight(root.left);
		}
		if(root.right != null) {
			rH = findingheight(root.right);
		}
		if(lH > rH) {
			return 1 + lH;
		}
