...
public class BST<T extends Comparable<? super T>> {
    ...
    public String getPath(T t1) {
    	...
    	else return getPathHelper(t1, root);
    }//End of getPath method
    
    // Exercise 1 (Helper method)
    public String getPathHelper(T t1, BSTNode<T> p) {
    	
    	if(( p.left == null && p.right == null ) || p.el == t1 ) {
    		if(p.el == t1) {
    			return ""+p.el;
    		}
    		else {
    			return " { The element ' "+t1+" ' is not in the tree }";
    		}
    		
    	}
    	else {
    		if(p.el.compareTo(t1) < 0) {
    			return p.el + " " + getPathHelper(t1, p.right);
    		}
    		else {
    			return p.el + " " + getPathHelper(t1, p.left);
    		}
    		
    	}
    }//End of getPathHelper method
    
    // Exercise 2
    public int getRightLeafCount() {
    	if(root == null) return 0;
    	
    	else return getRLCHelper(root);
    }//End of getRightLeafCount method
    
    // Exercise 2 (Helper method)
    public int getRLCHelper(BSTNode<T> p) {
		...
		if(p == null)
			...
			if(p.right.right == null && p.right.left == null)
				return 1 + getRLCHelper(p.left) + getRLCHelper(p.right);
			else
				return getRLCHelper(p.left) + getRLCHelper(p.right);
		}
    ...
    }//End of getRLCHelper method
    
    // Exercise 3
    public int rangeCounter(T t1, T t2) {
    	return rangeCounterHelper(root, t1, t2);
    }//End of rangeCounter method
    
    // Exercise 3 (Helper method)
    public int rangeCounterHelper(BSTNode<T> p,T greaterThan, T lessThan) {
    	
    	if(p == null) {
    		return 0;
    	}
    	
    	else if(p.el.compareTo(lessThan) < 0 && p.el.compareTo(greaterThan) > 0) {
    		return 1 + rangeCounterHelper(p.left, greaterThan, lessThan) 
    					+ rangeCounterHelper(p.right, greaterThan, lessThan);
    	}
    	
    	else {
    		return rangeCounterHelper(p.left, greaterThan, lessThan) 
			+ rangeCounterHelper(p.right, greaterThan, lessThan);
    	}
    
    
    }//End of rangeCounterHelper method
